...
class MapInstrument extends ISvgMapRootElement {
    constructor() {
        ...
        this.shownVorsIdentList = [];
        this._ranges = [0.5, 1, 2, 3, 5, 10, 15, 20, 35, 50, 100, 150, 200];
        this.rangeIndex = 4;
        this._declutterLevel = 0;
        this.rangeFactor = 1852;
        this.topViewAltitudeLevel = 0;
        this.topViewPitch = 90;
        this.topViewBank = 0;
        this.topViewRelativeHeading = 0;
        this.wpIdValue = "";
        this.wpDtkValue = "";
        this.wpDisValue = "";
        this.gsValue = "";
        ...
        this.bIsFlightPlanWaypointsVisible = true;
        this.mapRotationMode = EMapRotationMode.NorthUp;
        this.lastRotationMode = EMapRotationMode.NorthUp;
        this.maskElements = [];
        ...
        this.bShowDebugDistanceCircle = false;
        this.debugDistanceCircle = 0;
        this.overdrawFactor = 1;
        this._currentVor = null;
        this._currentWaypoint = null;
        this.zoomAxisValue = 0;
        this.zoomAxisThreshold = 1.7;
    ...
    connectedCallback() {
        ...
        if (this.hasAttribute("overdrawFactor")) {
            this.overdrawFactor = parseFloat(this.getAttribute("overdrawFactor"));
        }
        this.lineCanvas = document.createElement("canvas");
        this.lineCanvas.id = "line-canvas";
        this.lineCanvas.style.position = "absolute";
        this.lineCanvas.style.top = "-" + (this.overdrawFactor - 1) * 50 + "%";
        this.lineCanvas.style.left = "-" + (this.overdrawFactor - 1) * 50 + "%";
        this.lineCanvas.style.width = fastToFixed((this.overdrawFactor * 100), 0) + "%";
        this.lineCanvas.style.height = fastToFixed((this.overdrawFactor * 100), 0) + "%";
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        ...
        if (lowercaseName == "bing-mode") {
            ...
            else if (attr === "topview") {
                this.eBingMode = EBingMode.TOPVIEW;
            }
            else {
                this.eBingMode = EBingMode.PLANE;
            }
            ...
            var attr = newValue.toLowerCase();
            if (attr === "aerial") {
                this.eBingRef = EBingReference.AERIAL;
            }
            else if (attr === "plane") {
        ...
        else if (lowercaseName === "show-debug-distance-circle") {
            this.bShowDebugDistanceCircle = false;
            if (isFinite(newValue)) {
                this.bShowDebugDistanceCircle = true;
                this.debugDistanceCircle = parseFloat(newValue);
            }
        }
    }
    isInit() {
    ...
    init(arg) {
        ...
        if (!this.bingMap.is3D()) {
            ...
            this.navMap.overdrawFactor = this.overdrawFactor;
            this.navMap.instrument = this;
            ...
            this.roadNetwork.overdrawFactor = this.overdrawFactor;
            ...
            this.flightPlanElement.fp1UseSpecificColors = (SimVar.GetSimVarValue("L:MAP_FP1_NOT_SPECIFIC_COLORS", "Bool") == 0);
            ...
            this.directToElement = new SvgDirectToElement();
            this.debugCropSprayElement = new SvgCropSprayDebugElement("debug-crop-spray");
            this.debugCropSprayElement.corners = [
                new LatLong(36.203784, -119.582627),
                new LatLong(36.203819, -119.574978),
                new LatLong(36.207262, -119.574996),
                new LatLong(36.207221, -119.582708)
            ];
            this.debugCropSprayElement.subdivisions = 6;
            Coherent.call("RESET_ROAD_ITERATOR");
            this.addEventListener("mousedown", this.OnMouseDown.bind(this));
            this.addEventListener("mousemove", this.OnMouseMove.bind(this));
            this.addEventListener("mouseup", this.OnMouseUp.bind(this));
            this.addEventListener("mouseleave", (e) => { this.OnMouseLeave(); });
            Coherent.on("ForceMouseLeave", this.OnMouseLeave.bind(this));
            this.addEventListener("mouseenter", this.OnMouseEnter.bind(this));
            Coherent.on("ForceMouseEnter", this.OnMouseEnter.bind(this));
            SimVar.SetSimVarValue("L:" + this.bingId + "_MapShowCursor", "number", 0);
            SimVar.SetSimVarValue("L:" + this.bingId + "_MapZoomRange", "number", this.getDisplayRange());
            if (this.bShowDebugDistanceCircle) {
                this.createDebugDistanceCircle();
            }
        }
    ...
    updateFlightPlanVisibility() {
        ...
        if (this.flightPlanElement) {
            ...
            if (this.flightPlanManager)
                this.flightPlanManager.updateFlightPlanByIndex(EmptyCallback.Void, this.flightPlanElement.flightPlanIndex);
        }
    }
    onBeforeMapRedraw() {
        var _a, _b;
        if (!this.bingMap.is3D()) {
            ...
            if (this.showRoads) {
                ...
                while (this.roadsBuffer.length > 0 && (performance.now() - t0 < 0.2)) {
            ...
            if (needCenterOnPlane)
                ...
                this.scrollDisp.x *= 0.5;
                this.scrollDisp.y *= 0.5;
            ...
            this.navMap.mapElements.length = 0;
            if (this.debugCropSprayElement) {
                this.navMap.mapElements.push(this.debugCropSprayElement);
            }
            if (!this.isDisplayingWeatherRadar() || !this.weatherHideGPS) {
                if (this.showRoads || this.showAirways || this.showAirspaces) {
                    this.navMap.pushMapElement(this.roadNetwork);
                }
                if (this.showTraffic) {
                    if (this.getDeclutteredRange() < this.npcAirplaneMaxRange) {
                        this.navMap.pushMapElement(...this.npcAirplaneManager.npcAirplanes);
                    }
                }
                if (this.bShowAirplane) {
                    this.navMap.pushMapElement(this.airplaneIconElement);
                }
                if (this.showObstacles && this.navMap.centerCoordinates) {
                    if (Math.abs(this.navMap.centerCoordinates.lat - 47.6) < 2) {
                        if (Math.abs(this.navMap.centerCoordinates.long + 122.3) < 2) {
                            this.navMap.pushMapElement(...this.dummyObstacles);
                        }
                    }
                }
                let margin = 0.05;
                        ...
                        if (airport && airport.infos instanceof AirportInfo) {
                            if (this.navMap.isLatLongInFrame(airport.infos.coordinates, margin)) {
                                if (this.getDeclutteredRange() < this.smallAirportMaxRange) {
                                    this.navMap.pushMapElement(airport.getSvgElement(this.navMap.index));
                                }
                ...
                if (this.showVORs && (this.getDeclutteredRange() < this.vorMaxRange || this.getDeclutteredRange() < this.minimizedVorMaxRange)) {
                    for (let i = 0; i < this.vorLoader.waypoints.length; i++) {
                        ...
                        let displayed = false;
                        if (this.shownVorsIdentList.length > 0) {
                            for (let j = 0; j < this.shownVorsIdentList.length; j++) {
                                if (this.shownVorsIdentList[j] === vor.ident) {
                                    displayed = true;
                                    break;
                                }
                            }
                        ...
                        if (displayed && this.navMap.isLatLongInFrame(vor.infos.coordinates, margin)) {
                            let e = vor.getSvgElement(this.navMap.index);
                            this.currentVor = e;
                            if (e) {
                                e.forceNoMinimize = false;
                                this.navMap.pushMapElement(e);
                            }
                        }
                    }
                }
                if (this.showNDBs && (this.getDeclutteredRange() < this.ndbMaxRange || this.getDeclutteredRange() < this.minimizedNdbMaxRange)) {
                    for (let i = 0; i < this.ndbLoader.waypoints.length; i++) {
                        ...
                        if (this.navMap.isLatLongInFrame(ndb.infos.coordinates, margin)) {
                            let e = ndb.getSvgElement(this.navMap.index);
                            if (e) {
                                e.forceNoMinimize = false;
                                this.navMap.pushMapElement(e);
                            }
                ...
                if (this.showIntersections && (this.getDeclutteredRange() < this.intersectionMaxRange || this.getDeclutteredRange() < this.minimizedIntersectionMaxRange)) {
                    for (let i = 0; i < this.intersectionLoader.waypoints.length; i++) {
                        ...
                        if (this.navMap.isLatLongInFrame(intersection.infos.coordinates, margin)) {
                            let e = intersection.getSvgElement(this.navMap.index);
                            if (e) {
                                e.forceNoMinimize = false;
                                this.navMap.pushMapElement(e);
                            }
                ...
                if (this.showCities) {
                    for (let i = 0; i < this.cityManager.displayedCities.length; i++) {
                        ...
                        if (this.getDeclutteredRange() < this.smallCityMaxRange) {
                            this.navMap.pushMapElement(city);
                        }
                ...
                if (this.showConstraints) {
                    for (let i = 0; i < this.constraints.length; i++) {
                        this.navMap.pushMapElement(this.constraints[i]);
                    }
                }
                    ...
                    if (waypoint && waypoint.ident != "" && waypoint.ident !== "USER" && waypoint.ident !== "POI") {
                        if (waypoint.infos && this.navMap.isLatLongInFrame(waypoint.infos.coordinates, 0)) {
                            if (waypoint.getSvgElement(this.navMap.index)) {
                                let e = this.navMap.mapElements.find(w => {
                                    return (w instanceof SvgWaypointElement) && w.source.icao === waypoint.icao;
                                });
                                if (e instanceof SvgWaypointElement) {
                                    e.forceNoMinimize = true;
                                }
                                else {
                                    e = waypoint.getSvgElement(this.navMap.index);
                                    this.currentWaypoint = e;
                                    e.forceNoMinimize = true;
                                    this.navMap.pushMapElement(e);
                                }
                            }
                ...
                if (this.flightPlanManager && this.bIsFlightPlanVisible) {
                    if (SimVar.GetSimVarValue("L:MAP_SHOW_TEMPORARY_FLIGHT_PLAN", "number") === 1) {
                        this.navMap.pushMapElement(this.tmpFlightPlanElement);
                    ...
                    if (l > 1) {
                        this.navMap.pushMapElement(this.flightPlanElement);
                        if (this.bIsFlightPlanWaypointsVisible) {
                            for (let i = 0; i < l; i++) {
                        ...
                        if (apprWaypoint && apprWaypoint.ident !== "" && apprWaypoint.ident !== "USER") {
                            if (apprWaypoint.getSvgElement(this.navMap.index)) {
                                ...
                                if (e instanceof SvgWaypointElement) {
                                    ...
                                    e = apprWaypoint.getSvgElement(this.navMap.index);
                                    e.forceNoMinimize = true;
                                    this.navMap.pushMapElement(e);
                                }
                            }
                    ...
                    if (this.flightPlanManager.getIsDirectTo() && ((_b = (_a = this.flightPlanManager) === null || _a === void 0 ? void 0 : _a.getActiveWaypoint()) === null || _b === void 0 ? void 0 : _b.type) !== "A") {
                        ...
                        this.navMap.pushMapElement(this.directToElement);
                    }
                    if (this.tmpDirectToElement) {
                        this.navMap.pushMapElement(this.tmpDirectToElement);
                    }
                    this.navMap.pushMapElement(...this.backOnTracks);
                    if ((SimVar.GetSimVarValue("L:FLIGHTPLAN_USE_DECEL_WAYPOINT", "number") === 1) && this.flightPlanManager.decelWaypoint) {
                        this.navMap.pushMapElement(this.flightPlanManager.decelWaypoint.getSvgElement(this.navMap.index));
                    }
                    if (this.debugApproachFlightPlanElement) {
                        this.navMap.pushMapElement(this.debugApproachFlightPlanElement);
                    }
                    if (this.bShowDebugDistanceCircle) {
                        this.navMap.pushMapElement(this.circleSVGPath);
                    }
                }
                this.navMap.pushMapElement(...this.maskElements);
                this.navMap.pushMapElement(...this.topOfCurveElements);
                this.navMap.mapElements = this.navMap.mapElements.sort((a, b) => { return b.sortIndex - a.sortIndex; });
                if (this.bingMap && this.mapRotationMode != EMapRotationMode.KeepCurrent) {
                    let transform = "";
                    if (this.mapRotationMode != EMapRotationMode.NorthUp && !this.isDisplayingWeatherRadar()) {
                        var compass = 0;
        ...
        if (!this.bingMap.is3D()) {
            ...
            let setBingConfig = () => {
                if (svgConfigLoaded && this.instrument.isComputingAspectRatio()) {
                    svgConfig.generateBingMap(this.bingMap);
                    this.bingMap.setAspectRatio((this.instrument.isAspectRatioForced()) ? this.instrument.getForcedScreenRatio() : 1.0);
                    this.bingMap.setConfig(this.bingMapConfigId);
                }
                else {
    ...
    update(_deltaTime) {
        engine.beginProfileEvent("MapInstrument::doUpdate");
        ...
        if (this.wpt) {
            var wpId = Simplane.getGPSWpNextID();
            if (this.wpIdValue != wpId) {
                diffAndSetText(this.wpt, wpId);
                this.wpIdValue = wpId;
            }
        }
        if (this.dtkMap) {
            ...
            if (this.wpDtkValue != wpDtk) {
                diffAndSetText(this.dtkMap, wpDtk);
        ...
        if (this.disMap) {
            ...
            if (this.wpDisValue != wpDis) {
                diffAndSetText(this.disMap, wpDis);
                this.wpDisValue = wpDis;
            }
        }
        if (this.gsMap) {
            var gs = fastToFixed(Simplane.getGroundSpeed(), 0);
            if (this.gsValue != gs) {
                diffAndSetText(this.gsMap, gs);
                this.gsValue = gs;
            }
        }
        ...
        if (this.bingMap) {
            engine.beginProfileEvent("bingMap.setParams");
            if (this.eBingMode == EBingMode.TOPVIEW) {
                this.bingMap.setTopViewParams({ altitudeLevel: this.topViewAltitudeLevel, pitch: this.topViewPitch, bank: this.topViewBank, relativeHeading: this.topViewRelativeHeading });
            }
            else if (!this.bingMap.is3D()) {
                let bingRadius = this.navMap.NMWidth * 0.5 * this.rangeFactor;
    ...
    updateInputs() {
        engine.beginProfileEvent("updateInputs");
        if (this.eBingMode === EBingMode.VFR) {
            ...
            if (scrollX != 0 || scrollY != 0) {
                ...
                if (!this.isToolBarVFRMap) {
                    let quality = this.instrument ? this.instrument.getQuality() : -1;
                    if (quality === 0) {
                        this.svgSmooth -= 1;
                    }
                    else if (quality === 1) {
                    ...
                    this.svgSmooth = Math.max(1, this.svgSmooth);
                }
                this.bVfrMapPlanePositionReady = true;
                this.bVfrMapFollowPlane = false;
            }
        ...
        if (this.bMouseOver && !this.isToolBarVFRMapExtern()) {
            ...
            this.zoomAxisValue = this.zoomAxisValue + GetInputValue("INGAME_UI", "KEY_INGAMEPANEL_ZOOM_AXIS");
            if (this.zoomAxisValue > this.zoomAxisThreshold) {
                this.zoomIn();
                this.zoomAxisValue = 0;
            }
            else if (this.zoomAxisValue < -this.zoomAxisThreshold) {
                this.zoomOut();
                this.zoomAxisValue = 0;
            }
            else if (this.zoomAxisValue == 0) {
                this.zoomAxisValue = 0;
            }
        }
        engine.endProfileEvent();
    ...
    updateSize(_bForce = false) {
        if (_bForce || this.curWidth <= 0 || this.curHeight <= 0) {
            this.curWidth = this.clientWidth * this.overdrawFactor;
            this.curHeight = this.clientHeight * this.overdrawFactor;
        }
    }
    getWidth() {
    ...
    rotateWithPlane(_val) {
        this.mapRotationMode = _val ? EMapRotationMode.HDGUp : EMapRotationMode.NorthUp;
        if (this.navMap)
            this.navMap.rotationMode = _val ? EMapRotationMode.HDGUp : EMapRotationMode.NorthUp;
    }
    setRotationMode(_val) {
        if (this.mapOrientationElement) {
            switch (_val) {
                case EMapRotationMode.NorthUp:
                    diffAndSetText(this.mapOrientationElement, "NORTH UP");
                    break;
                case EMapRotationMode.DTKUp:
                    diffAndSetText(this.mapOrientationElement, "DTK UP");
                    break;
                case EMapRotationMode.HDGUp:
                    diffAndSetText(this.mapOrientationElement, "HDG UP");
                    break;
                case EMapRotationMode.TrackUp:
                    diffAndSetText(this.mapOrientationElement, "TRACK UP");
                    break;
            }
        }
        this.mapRotationMode = _val;
        if (this.navMap) {
            this.navMap.rotationMode = _val;
            if (this.roadNetwork) {
                this.roadNetwork.refreshRotation(this.navMap);
            }
        }
    }
    getRotationMode() {
        return this.mapRotationMode;
    }
    ...
    get currentVor() {
        return this._currentVor;
    }
    set currentVor(_currentVor) {
        this._currentVor = _currentVor;
    }
    get currentWaypoint() {
        return this._currentWaypoint;
    }
    set currentWaypoint(_currentWaypoint) {
        this._currentWaypoint = _currentWaypoint;
    }
    updateBingMapSize() {
        let w = this.curWidth;
        ...
        if (w * h > 1 && w * h !== this.lastWH) {
            ...
            this.bingMap.style.top = fastToFixed(((h - max) - h * (1 - 1 / this.overdrawFactor)) / 2, 0) + "px";
            this.bingMap.style.left = fastToFixed(((w - max) - w * (1 - 1 / this.overdrawFactor)) / 2, 0) + "px";
        }
    }
    ...
    getFlightPlanIsDashed() {
        if (this.flightPlanElement)
            return this.flightPlanElement.isDashed;
        return false;
    }
    setTmpFlightPlanAsDashed(_val) {
        if (this.tmpFlightPlanElement) {
            this.tmpFlightPlanElement.setAsDashed(_val);
        }
    }
    activateCursor() {
        if (EBingMode.VFR) {
        ...
        this.lastRotationMode = this.mapRotationMode;
        if (this.navMap) {
            this.navMap.rotationMode = EMapRotationMode.KeepCurrent;
        }
        this.mapRotationMode = EMapRotationMode.KeepCurrent;
        if (!this.bingMap.is3D()) {
            SimVar.SetSimVarValue("L:" + this.bingId + "_MapShowCursor", "number", 1);
        }
    }
    deactivateCursor() {
        ...
        if (this.navMap) {
            this.navMap.rotationMode = this.lastRotationMode;
        }
        this.mapRotationMode = this.lastRotationMode;
        this.hideCursor();
        if (!this.bingMap.is3D()) {
            SimVar.SetSimVarValue("L:" + this.bingId + "_MapShowCursor", "number", 0);
        }
    ...
    setCursorPos(x, y) {
        diffAndSetAttribute(this.cursorSvg, "style", "left:" + x + "%;top:" + y + "%;");
        this.cursorX = x;
        this.cursorY = y;
        this.bIsCursorDisplayed = true;
    }
    hideCursor() {
        diffAndSetAttribute(this.cursorSvg, "style", "display: none");
        this.bIsCursorDisplayed = false;
    }
    ...
    setCenteredOnPlane() {
        if (this.eBingMode != EBingMode.CURSOR) {
            diffAndSetAttribute(this, "bing-mode", "plane");
        }
        else {
            this.bWasCenteredOnPlane = true;
        }
    ...
    setNavMapCenter(_coordinates, _smoothness = 5) {
        if (this.navMap && _coordinates && isFinite(_coordinates.lat) && isFinite(_coordinates.long)) {
    ...
    scrollMap(_dispX, _dispY) {
        if (this.navMap.lastCenterCoordinates) {
            ...
            let movelong = _dispX * this.navMap.sinMapRightDirection + _dispY * this.navMap.sinMapUpDirection;
            let movelat = _dispX * this.navMap.cosMapRightDirection + _dispY * this.navMap.cosMapUpDirection;
            ;
            let long = -movelong * this.navMap.angularWidth / (1000 * scaleFactor);
            let lat = movelat * this.navMap.angularHeight / (1000 * scaleFactor);
    ...
    zoomIn() {
        this.setZoom(this.rangeIndex - 1);
    }
    zoomOut() {
        this.setZoom(this.rangeIndex + 1);
    }
    ...
    setTopViewAltitudeLevel(_index) {
        if (this.eBingMode == EBingMode.TOPVIEW) {
            let levels = (this.bingMap && this.bingMap.topView) ? (this.bingMap.topView.altitudeLevels) : 1;
            this.topViewAltitudeLevel = Utils.Clamp(_index, 0, (levels - 1));
        }
    }
    getTopViewAltitudeLevel() {
        if (this.eBingMode == EBingMode.TOPVIEW)
            return this.topViewAltitudeLevel;
        return 0;
    }
    setTopViewPBH(pbhInDegrees) {
        if (this.eBingMode == EBingMode.TOPVIEW) {
            if ((pbhInDegrees === null || pbhInDegrees === void 0 ? void 0 : pbhInDegrees.pitch) != null)
                this.topViewPitch = pbhInDegrees.pitch;
            if ((pbhInDegrees === null || pbhInDegrees === void 0 ? void 0 : pbhInDegrees.bank) != null)
                this.topViewBank = pbhInDegrees.bank;
            if ((pbhInDegrees === null || pbhInDegrees === void 0 ? void 0 : pbhInDegrees.relativeHeading) != null)
                this.topViewRelativeHeading = pbhInDegrees.relativeHeading;
        }
    }
    OnMouseDown(_e) {
        if (_e.button == 0) {
    ...
    getMapUpDirection() {
        if (this.navMap)
            return this.navMap.mapUpDirection;
        return 0;
    }
    getPlaneCoords() {
        let planePosition = new Vec2(0, 0);
        if (this.navMap.planeCoordinates) {
            this.navMap.coordinatesToXYToRef(this.navMap.planeCoordinates, planePosition);
        }
    ...
    createDebugDistanceCircle() {
        this.circleSVGPath = new SvgPathDebugElement("circle-debug-distance");
        let planeLLA = new LatLong(Simplane.getCurrentLat(), Simplane.getCurrentLon());
        let circleLLAS = [];
        for (let deg = 0; deg < 360; deg += 10) {
            circleLLAS.push(Avionics.Utils.bearingDistanceToCoordinates(deg, this.debugDistanceCircle, planeLLA.lat, planeLLA.long));
        }
        this.circleSVGPath.setWaypoints(circleLLAS);
    }
}
